...
export class FacilityController {
  ...
  createFacility = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const facility = await this.facilityService.facilityAdd(
      facilityDto(req.body),
      operatorId,
    );
  ...
  createFacilityReview = async (
    ...
    if (!Number.isInteger(facilityId)) {
      return next(
        new IdInvalidError("시설 ID가 유효하지 않습니다.", facilityId),
      );
    }
    const review = await this.facilityService.facilityReviewAdd(
  ...
  getFacilityList = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const facilities = await this.facilityService.facilityListGet(
      cursor,
      regionId,
      isReservable,
      isPublic,
